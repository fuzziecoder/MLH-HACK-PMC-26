...
function SlideTransition({ children, direction, onHeightReady }) {
    ...
    return (
        <motion.div ref={containerRef} custom={direction} variants={stepVariants} initial="enter" animate="center" exit="exit"
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }) {
    ...
    return (
        <motion.div onClick={() => { if (step !== currentStep && !disableStepIndicators) onClickStep(step); }} className="step-indicator" animate={status} initial={false}>
            <motion.div variants={{
                inactive: { scale: 1, backgroundColor: '#1a1a1a', color: '#555' },
                active: { scale: 1, backgroundColor: '#2a2a2a', color: '#e0e0e0' },
                complete: { scale: 1, backgroundColor: '#333', color: '#e0e0e0' }
            }} transition={{ duration: 0.3 }} className="step-indicator-inner">
                {status === 'complete' ? (
    ...
    return (
        <div className="step-connector">
            <motion.div className="step-connector-inner"
                variants={{ incomplete: { width: 0, backgroundColor: 'transparent' }, complete: { width: '100%', backgroundColor: '#444' } }}
